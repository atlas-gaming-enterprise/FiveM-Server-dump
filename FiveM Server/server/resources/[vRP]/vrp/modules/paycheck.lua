...
AddEventHandler('paycheck:salary', function()
	...
	if vRP.hasPermission(user_id,"Agent.paycheck") then
		vRP.giveBankMoney(user_id,5000)
		vRPclient.notifyPicture(source,{"CHAR_BANK_MAZE",1,"Federal Goverment",false,"Payday: ~g~$5000"})
	end	
	if vRP.hasPermission(user_id,"Admin.paycheck") then
		vRP.giveBankMoney(user_id,300)
		vRPclient.notifyPicture(source,{"CHAR_BANK_MAZE",1,"Admin",false,"Payday: ~g~$300"})
	end	
	if vRP.hasPermission(user_id,"ATLAS.paycheck") then
		vRP.giveBankMoney(user_id,8000)
		vRPclient.notifyPicture(source,{"CHAR_BANK_MAZE",1,"Private Security",false,"Payday: ~g~$8000"})
	end	
	-- EMS Paychecks	
	...
	if vRP.hasPermission(user_id,"security.paycheck") then
		vRP.giveBankMoney(user_id,2000)
		vRPclient.notifyPicture(source,{"CHAR_BANK_MAZE",1,"Security Paycheck",false,"Payday: ~g~$2000"})
	end
	end
end)
